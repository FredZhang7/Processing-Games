...
int paddleHeight= 20;
int paddleWidth= 70;
color paddleColor= color(221, 125, 15);
...
void setup() {
  ...
  b=loadFont("AGaramondPro-Italic-48.vlw");
  img=loadImage("winner.jpg");
  img2=loadImage("loser.jpg");
  img3=loadImage("nice.jpg");
  x=0;
  y=0;
  dx = 5; 
  dy = 0;
...
void draw() {
  if(lives>0){
    ...
    fill(242, 203, 29);
    textFont(a);;
    text("Paddle Game",80,300);
    textFont(b);;
    text("by Fred", 130,340);
    noStroke();
    image(img3,x,y);
    ...
    fill(255,0,0);
    image(img2,-38,70);
    displayText("Too sad, you lost.",width/4,height/2,false);
  }
  if (BasketOfBricks.size()==0){
    background(0,0,0);
    fill(255,255,0);
    image(img,-80,100);
    displayText("You're the beast",width/4,height/2,false);
  }
}
